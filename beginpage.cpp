...
void BeginPage::loginSuccess()
{
    ...
    playlist->addMedia(QUrl("../myPushBox/res/dio/back_music.mp3"));
    playlist->setPlaybackMode(QMediaPlaylist::Loop);
    player_music = new QMediaPlayer(this);
    player_music->setPlaylist(playlist);
    player_music->setVolume(20);
    player_music->play();
    //按钮
...
void BeginPage::seeLeader()
{
    ...
    while(query.next()){
        ...
        leaders[i]->setText(QString(QString("%1").arg(query.value(0).toString(),-27-query.value(0).toString().length()/2, ' ')
