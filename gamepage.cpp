...
GamePage::GamePage(QString user, int num, int style, bool sound, bool tip, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::GamePage)
{
    ui->setupUi(this);
...
void GamePage::tryMove(QPoint p1, QPoint p2){
    if(chessboard[p1.y()][p1.x()] == 0 || chessboard[p1.y()][p1.x()] == 2){
        ...
        move_times++;
        if(this->is_sound){
            QMediaPlayer *player_move = new QMediaPlayer(this);
            player_move->setMedia(QUrl("../myPushBox/res/dio/move.mp3"));
            player_move->setVolume(30);
            player_move->play();
        }
    }
...
void GamePage::isWin()
{
    ...
    if(this->is_sound){
        QMediaPlayer *player_win = new QMediaPlayer(this);
        player_win->setMedia(QUrl("../myPushBox/res/dio/win.mp3"));
        player_win->setVolume(30);
        player_win->play();
    }
    //提示框
        ...
        for(int j = 0; j < MAXN; j++){
            if(chessboard[i][j] == 3){
                ...
                if(lr && fb){
                    is_lose = true;
                    if(this->is_sound){
                        QMediaPlayer *player_lose = new QMediaPlayer(this);
                        player_lose->setMedia(QUrl("../myPushBox/res/dio/defeat.mp3"));
                        player_lose->setVolume(30);
                        player_lose->play();
                    }
                }
            }
